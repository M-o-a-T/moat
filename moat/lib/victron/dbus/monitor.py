...
class Service:  # noqa: D101
    whentologoptions = {
        ...
        "onIntervalOnlyWhenChanged",
        "onIntervalAlways",
        "never",
    }

    def __init__(self, id, serviceName, deviceInstance):  # pylint:disable=redefined-builtin
        super().__init__()
        self.id = id
...
class DbusMonitor(CtxObj):
    ...
    async def _ctx(self):
        ...
        async with _bus() as bus, anyio.create_task_group() as self._tg:
            ...
            await bus._init_high_level_client()  # noqa: SLF001
    ...
    async def _execute_value_changes(self, serviceName, objectPath, changes, options):
        ...
        await _call(
            ...
            options,
            changes,
            self.get_device_instance(serviceName),
        )
