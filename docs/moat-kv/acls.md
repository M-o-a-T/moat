...
accessed using a particular named ACL. Then you associate that ACL with
users that shall be bound by it.
...
Given the following data structure, the user "aclix" will only be able
to write initial data to `one` and `one two`. They can also read the
data back. However, any other access is not possible:
...
`tests/test_feature_acls.py::test_81_basic`, when dumped with the
command `moat kv : get -rd_`.
