...
class Unpacker:
    ...
    def _read_header(self):
        ...
        if b & 0b10000000 == 0:  # x00-x7F
            ...
            size, fmt, typ = _MSGPACK_HEADERS[b]  # ty:ignore[invalid-assignment]  # always 3-tuple for these keys
            self._reserve(size)
            if len(fmt) > 0:
            ...
            size, fmt = _MSGPACK_HEADERS[b]  # ty:ignore[invalid-assignment]  # always 2-tuple for these keys
            self._reserve(size)
            if len(fmt) > 0:
