...
def drills(a, b, d1=None, d2=None):
    ...
    w = a + 2 * d_out
    base = RY1 * stack(
        Rectangle(a * 7, w, align=(Align.MAX, Align.MIN)),
        tw,
    ...
    for d in range(int(a * 2), int(b * 2) - 1, -1):
        ...
        loc = 50 + 10 * d
        cyl = Loc(
            px, d_out + d / 2 if back else w - d_out - d / 2, 10 * back / sqrt(d) + 3.5
        ) * Cyl(d + 0.2, loc)
        base -= cyl
        if d1 is not None:
            d1 -= cyl
...
BS = 2.5
B = R3 * make_face(
    Polygon((0, -BS), (BS, 0), (BS, BS), (-BS, BS), (-BS, 0), (0, -BS), align=(None, None))
)
dr1 = 10
dr2 = 6.5
dr1w = dr1 + 2 * d_out
...
b2 = Loc(tw, 0, 0) * R2 * b1
h1, b1, b2 = drills(dr1, dr2 + 0.5, Loc(0, dr1w - b_off, 0) * b1, Loc(0, dr1w - b_off, 0) * b2)
