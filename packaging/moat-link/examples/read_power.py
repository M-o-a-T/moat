...
@click.option("--config","-c", type=click.File("r"), help="MoaT config file", default="/etc/moat/moat.cfg")
...
async def main(path,verbose,config,offset):
	...
	async with Link(CFG["link"]) as cli, httpx.AsyncClient() as s:
		while True:
			...
			async for v in get_data(s,series,tags,url = "http://a.rock.s:50081/api/query", t_start=t-datetime.timedelta(0,10000-offset), t_end=t+datetime.timedelta(2,offset)):
				if v.time+offset < tt:
					print("Skip:",v.time)
					val = v.value
					continue
