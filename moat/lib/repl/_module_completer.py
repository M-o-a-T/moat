...
class ModuleCompleter:
    ...
    def _find_modules(self, path: str, prefix: str) -> list[str]:
        if not path:
            ...
            builtin_modules = [
                name for name in sys.builtin_module_names if self.is_suggestion_match(name, prefix)
            ]
            third_party_modules = [
                module.name
        ...
        for segment in path.split("."):
            modules = [
                mod_info for mod_info in modules if mod_info.ispkg and mod_info.name == segment
            ]
            if is_stdlib_import is None:
                # Top-level import decide if we import from stdlib or not
                is_stdlib_import = all(self._is_stdlib_module(mod_info) for mod_info in modules)
    ...
    def _is_stdlib_module(self, module_info: pkgutil.ModuleInfo) -> bool:
        return (
            isinstance(module_info.module_finder, FileFinder)
    ...
    def iter_submodules(
        ...
        specs = [
            info.module_finder.find_spec(info.name, None) for info in parent_modules if info.ispkg
        ]
