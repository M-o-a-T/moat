...
async def main():
    async with Dbus() as bus, bus.service("com.victronenergy.battery.test.c") as srv:
        ...
        v0 = await srv.add_path(
            "/Dc/0/Voltage", V, gettextcallback=lambda p, v: "{:2.2f}V".format(v)
        )
        c0 = await srv.add_path(
            "/Dc/0/Current", 0.1, gettextcallback=lambda p, v: "{:2.2f}A".format(v)
        )
        p0 = await srv.add_path(
            "/Dc/0/Power", 0.2, gettextcallback=lambda p, v: "{:0.0f}W".format(v)
        )
        t0 = await srv.add_path("/Dc/0/Temperature", 21.0)
        mv0 = await srv.add_path(
            "/Dc/0/MidVoltage", V / 8, gettextcallback=lambda p, v: "{:0.2f}V".format(v)
        )
        mvd0 = await srv.add_path(
