...
class HostList(CtxObj):
    ...
    """
    def __init__(self, cfg:dict, link:Link, debug:bool=False, broadcaster:Broadcaster=None):
        self.link=link
        self.debug=debug
        self.cfg=cfg
    ...
    async def _mon_host(self):
        ...
        """
        async with self.link.d_watch(P("run.host"),subtree=True,meta=True,state=NotGiven) as mon:
            ...
            async for p,msg in mon:
                try:
                    ...
                    if h.data.get("p",None) != msg:
                        h.data.p = msg
                        changed = True
                    self.tg.start_soon(partial(h.trigger, _E.MSG_PING if msg["up"] else _E.MSG_DOWN, changed=changed))
                    # dprint("P    ",p,msg)
