    ...
    for prev_token, token, next_token in token_window:
        ...
        match token.type:
            ...
            case T.NAME:
                ...
                elif token.string in BUILTINS and not (
                    prev_token and prev_token.exact_type == T.DOT
                ):
                    span = Span.from_token(token, line_lengths)
...
def disp_str(
    ...
    if colors and colors[0].span.start < start_index:
        ...
        pre_color = theme[colors[0].tag] if theme else ""
    ...
    for i, c in enumerate(buffer, start_index):
        ...
        if colors and colors[0].span.end == i:  # current color ends now
            post_color = theme.reset if theme else ""
            colors.pop(0)
    ...
    if colors and colors[0].span.start < i and colors[0].span.end > i:
        ...
        chars[-1] += theme.reset if theme else ""
