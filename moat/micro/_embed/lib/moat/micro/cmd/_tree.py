...
class SubDispatch:
    ...
    def __init__(self, dispatch, path):
        for i,p in enumerate(path):
            try:
                dispatch = dispatch._sub[p]
            except KeyError:
                self._dest = dispatch
                self._rem = self.path[i:]
                break
        else:
            self._dest = dispatch
            self._rem = ()
    ...
    def _send(self, *a, **k):
        return self._dest.dispatch(self._rem + a, k)
    ...
    def __getattr__(self, k):
        ...
        Enables code like:
            s = d.get_sub("a","b")
            await s.r()
        which calls the subhandler at "a.b"'s `cmd_r` method.
        """
        if k[0] == "_":
            raise AttributeError(k)
        return partial(self._send, k)

    def __call__(self, *a, **k) -> Async:
        """
        Enables code like:
            s = d.get_sub("a","b")
            await s()
        which calls the subhandler at "a.b"'s `cmd` method.
        """
        if self._rem or a:
            return self._dest.dispatch(self._rem + a, k)
        else:
