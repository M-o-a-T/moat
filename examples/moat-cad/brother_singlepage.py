...
frame -= Rot(angle, 0, 0) * Box(
    2 * slot_x, 2 * slot_y, 2 * slot_z, align=(Align.CENTER, Align.MIN, Align.MAX)
)
...
frame += (
    Loc(hold_x / 2, 0, 0) * gbar
    + Loc(slot_x / 2 - wall + hold_x, 0, 0) * gbar
    + Loc(-slot_x / 2 + wall, 0, 0) * gbar
)
...
frame += (
    stab
    + Loc(paper_w / 4, 0, 0) * stab
    + Loc(-paper_w / 4, 0, 0) * stab
    + Loc(-paper_w / 2, 0, 0) * stab
)
