...
def _read_terminfo_file(terminal_name: str) -> bytes:
    ...
    for directory in _get_terminfo_dirs():
        ...
        hex_dir = "%02x" % ord(first_char)  # noqa:UP031
...
class TermInfo:  # noqa:D101
    ...
    def _parse_terminfo_file(self, terminal_name: str) -> None:
        ...
        magic, name_size, bool_count, num_count, str_count, str_size = struct.unpack(
            "<Hhhhhh", data[:offset]
        )
        ...
        capabilities = {}
        for cap, off in zip(_STRING_NAMES, string_offsets, strict=False):
            if off < 0:
...
def tparm(cap_bytes: bytes, *params: int) -> bytes:
    ...
    for match in reversed(matches):  # reversed to maintain positions
        ...
        result = result[: match.start()] + str(value).encode("ascii") + result[match.end() :]
