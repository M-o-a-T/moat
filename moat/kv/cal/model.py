...
class Calattr(ClientEntry):
        ...
        if force or src != self.watch_src:
            ...
            if src is not None:
                ...
                await self.root.err.record_working(
                    "Cal", self.subpath + ("write",), comment="dropped"
                )
        ...
        if force or dest != self.watch_dest or intv != self.watch_dest_interval:
            try:
                ...
                if dest is not None and intv is not None:
                    ...
                    await self.root.err.record_working(
                        "Cal", self.subpath + ("read",), comment="dropped"
                    )
            except RuntimeError as exc:
                await self.root.err.record_error("Cal", self.subpath + ("read",), exc=exc)
    ...
    async def dest_value(self, val):
        ...
        try:
            ...
            await self.root.err.record_working("Cal", self.subpath + ("read",))
                ...
                async with self.client.watch(
                    ...
                    async for msg in wp:
                        ...
                        try:
                            ...
                            if dev is None:
                                await self.root.err.record_error(
                                    "Cal", self.subpath + ("write",), comment="device missing"
                                )
                                return
                            await dev.set(*self.attr, value=val)
                            await self.root.err.record_working(
                                "Cal", self.subpath + ("write",), comment="write OK"
                            )

            except Exception as exc:
                await self.root.err.record_error("Cal", self.subpath + ("write",), exc=exc)
            finally:
                if self.watch_src_scope is sc:
                    self.watch_src_scope = None


class Calnode(ClientEntry):
    dev = None
    ...
    async def with_device(self, dev):
        """
        Called by the Cal monitor, noting that the device is now visible
