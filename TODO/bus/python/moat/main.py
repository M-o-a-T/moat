...
async def get_link_serial(obj, ser, **kw):
    ...
    t, b = await console_stack(
        UAStream(ser),
        log=obj.verbose > 2,
        reliable=not obj.reliable,
        console=0xC1 if obj.guarded else False,
        **kw,
    )
...
@click.option(
    ...
    type=int,
    default=115200,
    help="Baud rate to use (cfg.port.rate)",
)
...
@click.option(
    ...
    "--reliable",
    is_flag=True,
    help="Use Reliable mode, wrap messages in SerialPacker frame (cfg.port.reliable)",
)
@click.option(
    "-g",
    "--guarded",
    is_flag=True,
    help="Use Guard mode (prefix msgpack with 0xc1 byte, cfg.port.guard)",
)
