...
async def test_alarm(mock_clock):
    ...
    async with stdtest(test_0={"init": 125}, n=1, tocks=200) as st, st.client(0) as client:
        ...
        await st.run("ow attr -d 10.345678.90 -i 5 temperature test.foo.temp")
        await st.run("ow attr -d 10.345678.90 -w templow test.foo.low")
        await st.run("ow attr -d 10.345678.90 -w foo.bar -a bar:1 test.foo.what.ever")
