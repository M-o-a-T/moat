...
async def dump_(obj, one_line):
    ...
    if not one_line:
        await data_get(obj.client, obj.server._path, recursive=True)
        return
...
async def at_cli(ctx, path):
    ...
    if ctx.invoked_subcommand is None:
        await data_get(obj.client, obj.server._path + obj.subpath, recursive=False)
...
async def dump_at(obj, one_line):
    ...
    if one_line:
        await data_get(obj.client, obj.server._path + obj.subpath, recursive=True)
        return
