...
MoaT-Link stores its data in a hierarchical tree. The MQTT top level for
structured MoaT-Link data is configured as `link.root`. It **must not**
be empty.

Messages **must not** be sent with the Retain bit set unless
...
hierarchies. They are described in the
<span class="title-ref">doc/schema.yml</span> file (machine readable),
and in <span class="title-ref">doc/doc.yml</span> (for humans).
...
Documentation about MoaT command line usage is stored in
`moat.doc.shell`.
...
<span class="title-ref">schema/\_/frame</span>. In MQTT the metadata is
transmitted as an optional user property named 'MoaT'.
...
encoded in CBOR and Base85/btoa. The last entry *may* be a map of
free-form extension data not covered by this specification.
...
The second member is a float or integer, serving as a timestamp
(standard Unix time) and possibly tagged with CBOR Tag 1. A string *may*
be used instead; it *may* contain a time zone. If not, UTC is implied.
...
Print the documentation. Without arguments, prints this document.
Otherwise emits the text at the given path.
