...
class DbusService:  # noqa: D101
    ...
    async def setup_done(self):  # noqa: D102
        ...
        logger.info("registered ourselves on D-Bus as %s", self._servicename)
    ...
    async def add_path(  # noqa: D102
        ...
        logger.debug("added %s with start value %s. Writeable is %s", path, value, writeable)
        return item
...
class ServiceContext:  # noqa: D101
    ...
    async def set(self, var, newvalue):  # noqa: D102
        c = await var._set_value(newvalue)  # noqa: SLF001
        if c is not None:
            self.changes[var._path] = c  # noqa: SLF001
...
class DbusItemImport:  # noqa: D101
    ...
    def __init__(self, bus, serviceName, path, eventCallback=None, createsignal=True):
        ...
        assert eventCallback is None or createsignal
        self._bus = bus
        self._serviceName = serviceName
        self._path = path
        self._match = None
...
class DbusTreeExport(dbus.ServiceInterface):  # noqa: D101
    def __init__(self, service, path):
        ...
        self._path = path
        logger.debug("DbusTreeExport %r has been created", path)

    async def _get_value_handler(self, path, get_text=False):
        logger.debug("_get_value_handler called for %s", path)
        r = {}
        px = path
        if not px.endswith("/"):
            px += "/"
        for p, item in self._service._dbusobjects.items():  # noqa: SLF001
            if p.startswith(px):
...
class DbusRootExport(DbusTreeExport):  # noqa: D101
    ...
    async def GetItems(self) -> "a{sa{sv}}":  # noqa: D102,F722
        return {
            ...
            for path, item in self._service._dbusobjects.items()  # noqa: SLF001
        }
...
class DbusItemExport(dbus.ServiceInterface):  # noqa: D101
    ...
    async def close(self):  # noqa: D102
        ...
        logger.debug("DbusItemExport %s has been removed", self._path)
