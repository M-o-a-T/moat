...
def side_corner():
    return corner()+Rot(0,0,90)*corner()+Loc(-corner_r,0,0)*R3*RX1*extrude(base_profile_nf,-corner_r*2)
C3=side_corner()
...
def bar():
    ...
    #bar=Loc(grid/5, -ib,0)*Box(grid/6.5,bar_w/2-ib,bar_h*.9, align=(Align.MIN,Align.MAX,Align.MIN))
    #bar=chamfer(bar.edges().filter_by(Axis.X).group_by(Axis.Z)[-1].group_by(Axis.Y)[0],1)
    
    f1=Polygon((0,ib),(0,wb),(hb-ch,wb),(hb,wb-ch),(hb,ib),(0,ib), align=(None,None))
    f2=Polygon((0,bar_w/x3),(0,wb),(hb-ch,wb),(hb,wb-ch),(hb,bar_w/x3),(0,bar_w/x3), align=(None,None))
...
def nose():
    ...
    nose += R3*C3-Loc(w2+.1,0,0)*R2*Box(bar_w+.1,2*corner_r,bar_h,align=(Align.MIN,Align.CENTER,Align.MIN))
    
    nose -= Loc(0,-corner_r,0)*Box(2*corner_r,.1,bar_h,align=(Align.CENTER,Align.MIN,Align.MIN))
    nose -= Loc(0,corner_r,0)*Box(2*corner_r,.1,bar_h,align=(Align.CENTER,Align.MAX,Align.MIN))
    return nose
...
def slot_xy(d=0,h=slot_h):
    ...
    """
    #((1.4,3),(1.4,2.2),(3.25,0.3) )
    slot=Rot(0,0,-45)*RX1*Loc(0,0,-.8*nose_len+d)*stack(R(3.5-3*d,h), .75*nose_len-2*d, R(1.4-2*d,h),nose_len*.3+2*d, None, nose_len*1.5,R(nose_len*20,bar_h*1.5))
    return slot
...
def base(x,y):
    """
    This is your basic x*y baseplate. It has screw holes in the center corners and connector slots around the edge.
    """
    b=bar()
    C4S=C4-screw()
    C3S=C3-slot()
...
def end_cut():
    ...
    return Loc(corner_r,corner_r,0)*Cylinder(corner_r+w2+.1, bar_h,align=(Align.CENTER,Align.CENTER,Align.MIN))
...
def zigzag_bars(right:bool=True):
    ...
    Used in place of the endboxed_square when your rows got offset by one due to botched planning. ;-)
    """
