...
document it's obvious that when a node increments its `tick` but the
associated data gets lost (e.g. if the node or its Serf agent crashes),
you have a problem.
...
when written to. TODO: allow the server to be in maintainer-only mode
when that happens.
...
`known` list. Calling `moat kv internal state -m` verifies that the list
is now empty.

Use the `--broadcast` flag to send this message to all MoaT-KV servers,
not just the one you're a client of.

This action will allow the bad record to re-surface when the node that
has the record reconnects, assuming that there is one. You can use the
`mark` command's `--deleted` flag to ensure that it will be discarded
instead.
