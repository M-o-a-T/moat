...
_rand_choices: Callable[..., list[str]] = random.choices

rs = os.environ.get("PYTHONHASHSEED", None)
if rs is not None:  # pragma: no cover
    try:
        tcr = import_module("trio._core._run")
    except ImportError:
        pass
    else:
        r = getattr(tcr, "_r", None)
        choices = None if r is None else getattr(r, "choices", None)
        if callable(choices):
            _rand_choices = choices
...
def gen_ident(k: int = 10, /, *, alphabet: str = al_unique) -> str:
    ...
    """
    return "".join(_rand_choices(alphabet, k=k))
