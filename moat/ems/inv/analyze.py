...
class InvMode_Analyze(InvModeBase):
    ...
    @property
    def t_balance(self):
        "Time to hold the battery in top balancing. -1=do not balance."
        return self.intf.op.get("balance", 0)

    @property
    def use_grid(self):
        "Flag: power values refer to the grid, not the inverter."
        return self.intf.op.get("use_grid", False)

    @property
    def skip(self):
        "Skip the first N processing steps."
        return self.intf.op.get("skip", 0)

    @property
    def e_dis_in(self):
        "Discharge energy (Ws), if step 4 is skipped."
        return self.intf.op.get("e_dis", 0)

    @property
    def e_chg_in(self):
        "Charge energy (Ws), if step 5 is skipped."
    ...
    _doc = dict(
        ...
        balance=t_balance.__doc__,
        skip=skip.__doc__,
        e_dis=e_dis_in.__doc__,
        e_chg=e_chg_in.__doc__,
        use_grid=use_grid.__doc__,
        _l="""\
