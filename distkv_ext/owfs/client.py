...
async def set_(obj,device,family, value,eval_,name,split):
    ...
    res = await node_attr(obj, obj.cfg.owfs.prefix + fd, name, value=value, eval_=eval_, split_=split)
    if res and obj.meta:
        yprint(res, stream=obj.stdout)
...
async def server(obj, name, host, port, delete):
    ...
    if not name:
        ...
        async for r in obj.client.get_tree(obj.cfg.owfs.prefix|"server", min_depth=1, max_depth=1):
    ...
    if host or port:
        ...
        value = None
    res = await node_attr(obj, obj.cfg.owfs.prefix|"server"|name, P("server"), value, eval_=False)
    if res and obj.meta:
        yprint(res, stream=obj.stdout)
