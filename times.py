		...
		if unit in ("s","sec","second","seconds"):
			...
			if val > 0 and val < 100:
				...
				if val < now.year or val >= now.year+100:
					raise SyntaxError("WHICH year? Sorry, the time machine module is not available.")
			p.yr = val
			...
			if val == 0 or abs(val) > 4:
				raise SyntaxError("Months have max. 5 of each weekday; use -1 if you want the last one.")
			p.dow = weekdays[unit]
			p.nth = val
			continue
		else:
