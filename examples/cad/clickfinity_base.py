...
base_profile = Polygon((0, 0), (w1, 0), (w2, h1), (w2, h2), (0, h3), (0, 0), align=(None, None))

base_profile_nf = Polygon((0, 0), (w2, 0), (w2, h2), (0, h3), (0, 0), align=(None, None))
...
def outer_corner():  # noqa:D103
    return (
        R2
        * Loc(-corner_r, -corner_r, 0)
        * RX1
        * revolve(Loc(corner_r, 0, 0) * base_profile_nf, Axis.Y, 90)
    )
...
def bar():
    ...
    cut = Loc(grid / 5, bar_w / 15, 0) * Box(
        grid / 4, bar_w, bar_h, align=(Align.MIN, Align.MAX, Align.MIN)
    )
    cut = chamfer(cut.edges().filter_by(Axis.Z), bar_w / 6)
    b = b - cut - Loc(grid, 0, 0) * R2 * cut

    bar_profile = Polygon(
        (ib, 0), (ib, bar_h - ib), (2.15, bar_h - 2.15), (2.15, 0), (ib, 0), align=(None, None)
    )
    ...
    f1 = Polygon(
        (0, ib), (0, wb), (hb - ch, wb), (hb, wb - ch), (hb, ib), (0, ib), align=(None, None)
    )
    f2 = Polygon(
        (0, bar_w / x3),
        (0, wb),
...
def nose():
    ...
    nose += R3 * C3 - Loc(w2 + 0.1, 0, 0) * R2 * Box(
        bar_w + 0.1, 2 * corner_r, bar_h, align=(Align.MIN, Align.CENTER, Align.MIN)
    )

    nose -= Loc(0, -corner_r, 0) * Box(
        2 * corner_r, 0.1, bar_h, align=(Align.CENTER, Align.MIN, Align.MIN)
    )
    nose -= Loc(0, corner_r, 0) * Box(
        2 * corner_r, 0.1, bar_h, align=(Align.CENTER, Align.MAX, Align.MIN)
    )
    return nose
...
def slot_xy(d=0, h=slot_h):
    ...
    slot = stack(
        ...
        None,
        nose_len * 1.5,
        R(nose_len * 20, bar_h * 1.5),
    )
    slot = Rot(0, 0, -45) * RX1 * Loc(0, 0, -0.8 * nose_len + d) * slot
    return slot
...
def nose_xy(right: bool | None = None):
    ...
    bx = rot(end_cut())
    res = (RC - bx) + (bx & nxy)
    res -= Loc(0, -corner_r, 0) * Box(
        2 * corner_r, 0.1, bar_h, align=(Align.CENTER, Align.MIN, Align.MIN)
    )
    res -= Loc(-corner_r, 0, 0) * Box(
        0.1, 2 * corner_r, bar_h, align=(Align.MIN, Align.CENTER, Align.MIN)
    )
...
def end_cut():
    ...
    return Loc(corner_r, corner_r, 0) * Cylinder(
        corner_r + w2 + 0.1, bar_h, align=(Align.CENTER, Align.CENTER, Align.MIN)
    )
